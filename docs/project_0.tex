...
\end{figure}
%TODO In figuur hierboven misschien ook nog added value services toevoegen, en misschien external data sources gewoon hernoemen naar validated data sources.
...
%TODO misschien beter zeggen identificiëren van gebruikers in het systeem, er is niet echt een centrale database.
...
%TODO In de figuur hierboven is er een poort te zien die naar de interface grantAccess gaat, deze ontbreekt bij het overzichts-figuur, ook mis ik daar de readExternal required interface
...
%TODO meer een vraagje dan iets dat nog moet gebeuren : waarom asynchroon?
...
%TODO misschien meer uitleg wat die patch juist is, ik denk niet dat een gewone gebruiker van het systeem weet waar het hier over gaat, aan de andere kant is deze informatie misschien ook wel meer gericht op de tester en niet op de patiënt
id\_key: de key die gebruikt wordt om de uitvoerder van de actie op te vragen.\\
session\_key: de key die gebruikt wordt om een sessie te identifici\"{e}ren.\\
...
%TODO Dit hierboven komt niet overeen met wat er in het deployment view staat. Daar komen alle inkomende berichten van de security component, die daarvoor al alle authenticatie en authorisatie heeft gedaan. Dat is ook een meer modulaire aanpak omdat je je zo niet echt veel moet aantrekken van security in die GMR component. Zou dit nog aangepast kunnen worden? Ik denk niet dat ik het zoals hier kan aanpassen in het deployment view. De security component voorziet ook een firewall, dus hier zal alle inkomende verkeer eerst van moeten komen en aangezien de security component en de GMR component fysiek gescheiden zijn, zou het stom zijn om nog eens extra communicatie heen en weer te laten gaan.
...
%TODO In deployment view wordt hier niet veel over gezegd. En ik denk dat je hier gewoon makkelijk het volgene bij kan zetten : 
%
...
%TODO In deployment staat wel wat over toegang tot data in de security data server, maar niet echt iets over de inhoud daarvan, dus misschien bovenstaande referentie gewoon weglaten?
...
%FIXME Misschien eerder de rol van de gebruiker teruggeven, nu is het niet echt duidelijk wat er juist teruggegeven wordt.
